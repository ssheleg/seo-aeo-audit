...
adversarial material lives in threats-and-defense.md as defence only.
...
| B5 | Broad `robots.txt` wildcard (`/*?`, `/*print`) | Replace with parameter-specific rules; canonicalise filtered variants | Wildcards match substrings, not paths | One store recovered ~40% of product pages; a retailer's 45% traffic loss traced to one line | FIELD |
...
| L7 | Orphans (crawl ∖ sitemap diff) | Link them from relevant hubs or retire them deliberately | PageRank decays ~85% per hop; zero-click pages get deprioritised | Deep pages need ≥1 click/week to hold their index slot | FIELD |
...
| G5 | Brand described differently across profiles | Normalise 8–12 third-party profiles to the homepage reference; re-check quarterly | Conflicting category signals blur the brand centroid | Prerequisite for consistent AI answers | STUDY |
...
| G11 | Long-form asset with no distribution | Atomise into ~60 platform-native pieces, each linking back, released over weeks | Distribution doubles as equity and independent entry points | Reported ~14× reach at zero content cost | FIELD |
| G12 | Local/service business with generic targeting | Neighbourhood, landmark, urgency and "near me + qualifier" keyword layers built from Maps, PAA, autocomplete, GBP insights, support tickets | Specific intent beats broad location terms | Aggregate 1,840/mo at low competition, positions 2–5, 3× conversion vs the head term | FIELD |
...
| G15 | Agentic surfaces unaddressed | Expose WebMCP tools (`/mcp/` page listing them, footer link, referenced from `llms.txt`) | Agents call structured tools instead of guessing from screenshots | Early-mover positioning; no ranking claim | HYPOTHESIS |
