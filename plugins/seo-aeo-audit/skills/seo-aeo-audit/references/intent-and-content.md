...
Validation is behavioural, not aesthetic: bounce <40% and time on page >2 min
...
negative = you are optimising a shrinking market and should say so in the report.
...
volatility; the winner stabilises first and combined clicks rise once the system
sees one source.
...
- **Medium (labour moat)**: hands-on reviews with real testing, insider expert
...
- Per-target-keyword: the top-10 format inventory, your page type, the mismatch
  verdict, and current behavioural metrics.
