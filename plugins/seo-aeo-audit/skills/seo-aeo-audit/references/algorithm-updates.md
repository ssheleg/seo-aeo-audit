...
| The opt-out's likely cost to publishers: Top Stories shown **inside** an AI Overview | reported 2026-07-30 | Roughly one in six US trending-news queries now places Top Stories inside the AI Overview, and the practitioner reading is that the generative opt-out removes a publisher from that embedded placement while leaving standalone Top Stories eligibility intact — with no guarantee the standalone module appears on the same page. Google's help text does not say how the control treats a classic feature rendered inside an AI one (`HYPOTHESIS` — practitioner analysis, unconfirmed by Google). Price it before recommending the opt-out to any news property. |
