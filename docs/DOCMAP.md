...
| The bundled script inventory | the `scripts/` directory | `SECURITY.md` states the count in five sentences and one row per script; `SKILL.md` names them in its inventory paragraph — all measured against the directory since B-17, which had six/22 against seven/26 for four releases, and a `sitemap_pull.py` that has never existed |
