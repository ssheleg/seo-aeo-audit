...
| The schema each manifest is checked against | `SCHEMA_FOR` in `test/check_schemas.py` | the `$schema` at the root of each manifest, and the offline guard in `validate.py` — both read the map rather than restating it. The addresses themselves are SchemaStore's; `claude-code-plugin.json` is a 404 and is refused by name |
...
| Declare or change a `$schema` | the pinned address in `test/check_schemas.py:SCHEMA_FOR`, and a planted-defect step per branch | `validate.py` **declared schemas** guard pins the address, the document type and the ROOT position offline; `test/check_schemas.py` fetches each address and validates the document against what is served, from CI. The map has one home and both halves import it — two checks with two copies of a URL is how the copies disagree |
