...
A run produces more than a diff. Four of this repository's Python suites built temp trees
and removed none of them — `test/plant_guard_test.py:34` (one per case, nine cases),
`test/test_output_contracts.py:309`, `test/test_page_audit.py:240` and
`test/test_collectors.py:128` — and, the part that matters, nothing said so. Measured on
2026-08-20 under this machine's `$TMPDIR`, with this repository the LAST of four in the
family still leaking the shared fixture:

    find "$TMPDIR" -maxdepth 4 -type f -path '*/copy/sub/b.sh' | wc -l   # 2560
    find "$TMPDIR" -maxdepth 2 -type d -name 'tmp*' | wc -l              # 28343
    du -sh "$TMPDIR"                                                     # 7.0G

The plant-guard fixture is byte-identical in four repositories of this family
(md5 `623a086d10a04940573c31cbebb93e31`), so those 2560 trees cannot be attributed to any
one of them — which is exactly why they are reported and **left in place** rather than
swept: sweeping state you cannot prove you own is what `manifesto.md:366` refuses.
`make-skill` closed its own copy on 2026-08-19 and filed the referral (MS-02), the
umbrella closed its own on 2026-08-20, and this is the last one. The module is **copied
from those repositories rather than rewritten**, because two implementations of one
ledger is the defect this family keeps finding in its own history.
...
The prefix is part of the mechanism: `seo-aeo-audit-test-…` makes any future residue
attributable to this suite by name. The 2560 directories measured above are plain
