...
def a_passing_case_leaves_nothing():
    """THE LEAK. `test/plant_guard_test.py:34` built one tree per case and removed none."""
...
def the_prefix_makes_residue_attributable():
    """The 2560 trees measured here are plain `tmpXXXXXXXX` — nameless, so unswept."""
    proc, paths = drive(FAILING)
    try:
        assert len(paths) == 1, proc.stdout
...
def the_four_leaking_suites_leave_nothing():
    """The four suites the 2026-08-20 measurement named, run for real in a private TMPDIR.

    `plant_guard_test.py` built a tree per case and the other three one each, and none of
    them removed anything — 2560 trees of the shared plant-guard shape were on this
    machine when the count was taken, across the four repositories that ship the fixture
    byte for byte. Each suite is run here in a box of its own and the box must be empty
    afterwards, which is a stronger claim than "the ledger says nothing was left": the
    ledger is what is under test.

    The marker differs per suite because only `plant_guard_test.py` counts its cases in
    its PASS line; asserting a count these others never print would be asserting the
    wording of a message rather than that the suite ran.
    """
    for suite, marker in (("plant_guard_test.py", "9 cases"),
                          ("test_output_contracts.py", "PASS: output contracts"),
                          ("test_page_audit.py", "PASS: page_audit behavior"),
                          ("test_collectors.py", "PASS: collector behavior")):
        box = residue.workspace("e2e-" + suite.replace(".py", ""))
        ...
        assert marker in proc.stdout, suite + " did not run: no %r in its output" % marker


def this_gate_run_left_nothing_behind():
    """Reads the TMPDIR the whole gate shares, after the two suites above have run in it.

    `bash scripts/check-docs.sh` runs this file last, in one shell, so every earlier suite
    used this `$TMPDIR`. A hit means a suite here is leaking again — or a second gate run
    is in flight, or somebody ran the workflow's plants in this shell: a planted run is
    DESIGNED to fail and a failing case keeps its tree, which is why `plant` and `plantq`
    give the planted interpreter a `TMPDIR` of its own. Eighteen trees were sitting here
    when that was found. All three causes are named because a check must not report a
